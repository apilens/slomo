...
The crash appears in `slomo issues` with its traceback and captured frame locals — same as a main-thread crash. See [`examples/background_worker.py`](https://github.com/bhatt-neel-dev/slomo/blob/main/examples/background_worker.py) for a complete runnable demo of worker-thread crashes you'd otherwise never see.
